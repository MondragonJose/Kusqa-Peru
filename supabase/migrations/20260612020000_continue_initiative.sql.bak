-- KUSQA Phase 4C.2: Continue dormant initiative — additive continuation event.
--
...
begin
  if not exists (
    ...
    where enumtypid = 'public.civic_event_kind'::regtype
      and enumlabel = 'initiative.continued'
  ) then
    alter type public.civic_event_kind add value 'initiative.continued';
  end if;
end $$;

-- ─── 2. RPC: continue_initiative ──────────────────────────────────────────
