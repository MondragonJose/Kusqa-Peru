...
4. **app.index.tsx:54**
   ...
   {currentUser?.xp?.toLocaleString() || "0"}  // ✓ Safe con fallback
   {currentUser?.peopleImpacted ? `${currentUser.peopleImpacted}+` : "0"}  // ✓ Safe
   ```
...
export function useCurrentUserState(): CurrentUserState {
  ...
  if (authState.state === "initializing") {
    ...
    status = error?.message.includes("No authenticated")
      ? "unauthenticated"
      : "error";
  } else if (user) {
