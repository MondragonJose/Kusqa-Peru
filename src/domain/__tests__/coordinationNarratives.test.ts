...
describe("buildNearbyActivityNarrative", () => {
  it("returns null when no activity", () => {
    const r: RelatedTerritorialActivity = {
      activityCount: 0,
      neighboringSlugs: [],
...
describe("buildCoalitionProximityNarrative", () => {
  it("returns null when no nearby coalitions", () => {
    const p: CoalitionProximity = {
      nearbyCoalitions: 0,
      nearbyCollaborators: 0,
      closestCoalitionDistance: "none",
    };
  ...
  it("returns narrative for near coalitions", () => {
    const p: CoalitionProximity = {
      nearbyCoalitions: 2,
      nearbyCollaborators: 4,
      closestCoalitionDistance: "near",
    };
...
describe("buildNeighboringAwarenessNarrative", () => {
  it("returns null when no neighbors", () => {
    const a: NeighboringDistrictAwareness = {
      totalNeighbors: 0,
      activeNeighbors: 0,
      dormantNeighbors: 0,
      neighborActivityRatio: 0,
    };
...
describe("deriveCoordinationNarratives", () => {
  it("returns empty for all-null inputs", () => {
    const related: RelatedTerritorialActivity = {
      activityCount: 0,
      neighboringSlugs: [],
      recentActivity: false,
      coordinationSignal: false,
    };
    const proximity: CoalitionProximity = {
      nearbyCoalitions: 0,
      nearbyCollaborators: 0,
      closestCoalitionDistance: "none",
    };
    const continuity: TemporalContinuity = {
      hasSustainedActivity: false,
      totalActiveWeeks: 0,
      gapWeeks: 0,
      pattern: "first_steps",
    };
    const awareness: NeighboringDistrictAwareness = {
      totalNeighbors: 0,
      activeNeighbors: 0,
      dormantNeighbors: 0,
      neighborActivityRatio: 0,
    };
    const emergence: AdjacentCoalitionEmergence = {
      emergingCoalitions: 0,
      coalitionSlugs: [],
      hasAdjacentCoalitions: false,
    };
    const missionContinuity: NeighboringMissionContinuity = {
      hasContinuity: false,
      contiguousMissions: 0,
      corridorForming: false,
    };

    const narratives = deriveCoordinationNarratives(
      related,
      proximity,
      continuity,
      awareness,
