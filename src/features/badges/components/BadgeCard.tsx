...
export function BadgeCard({ badge, index = 0, showNarrative = true }: BadgeCardProps) {
  ...
  return (
    ...
    >
      {/* Rarity indicator */}
      {badge.earned && (
        <div className={`absolute top-3 right-3 text-[9px] uppercase tracking-widest font-bold ${style.textClass}`}>
          {style.label}
        </div>
      )}
      ...
      {/* Narrative / condition */}
      {showNarrative && (
        <p className={`text-xs mt-2 leading-relaxed ${badge.earned ? "text-muted-foreground" : "text-muted-foreground/40"}`}>
      ...
      {/* Earned state */}
      {badge.earned && (
        <div className={`mt-3 inline-flex items-center gap-1 text-[10px] uppercase tracking-widest font-bold ${style.textClass}`}>
          <Sparkles className="h-3 w-3" />
          {badge.earnedAt
            ? new Date(badge.earnedAt).toLocaleDateString("es-PE", { month: "short", year: "numeric" })
            : "Desbloqueada"}
        </div>
      )}
