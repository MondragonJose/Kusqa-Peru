...
// Territorial Hierarchical Types
...
const TERRITORY_HIERARCHY: Record<string, TerritoryNode[]> = {
  root: [
      ...
      <div className="flex-1 relative h-[380px] lg:h-auto">
        ...
        {leafletLoaded && missions.length === 0 && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-card/90 z-10 backdrop-blur-sm p-6 text-center">
            ...
            <h3 className="font-display font-black text-lg text-foreground mb-2">Tu comunidad está despertando</h3>
            <p className="text-sm text-muted-foreground max-w-xs mb-4">
              Las misiones aparecerán aquí cuando jóvenes como tú comiencen a proponer acciones. Sé el primero en inspirar cambio.
            </p>
            <button
              onClick={() => window.location.href = '/app/crear'}
              className="px-4 py-2 rounded-xl bg-gradient-sunrise text-white text-xs font-black shadow-glow hover:scale-[1.02] transition-all"
            >
              Proponer la primera misión
            </button>
          </div>
        )}
