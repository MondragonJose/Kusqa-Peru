...
export function MissionStoryModal({ isOpen, onClose, missionId }: MissionStoryModalProps) {
  ...
  const timelineMilestones: StoryTimelineNode[] = [
    {
      ...
      description:
        "12 jóvenes y vecinos se reunieron en la plaza para trazar el mapa de necesidades locales, logrando consenso sobre el diseño final.",
      participants: ["Sayri Ccama", "Lucía Herrera", "Andrés Vega"],
      lessonsLearned:
        "Escuchar a los vecinos mayores antes de plantar nos enseñó cuáles especies resisten mejor las heladas locales.",
      emotionalNote:
        "Había mucha desconfianza al inicio, pero compartir pan y chicha de jora rompió el hielo.",
    },
    {
      title: "Remoción de Desechos e Incisión Cívica",
      date: "Día 7 de la Expedición",
      description:
        "Se retiraron 40 sacos de desmontes y plásticos acumulados. Se niveló el terreno para preparar la siembra colectiva.",
      participants: ["Joaquín Ríos", "Camila Díaz", "Sayri Ccama"],
      lessonsLearned:
        "Nivelar sin herramientas pesadas toma el triple de tiempo, pero fortalece la resistencia física del equipo.",
      emotionalNote:
        "Dos niños del barrio trajeron sus palas de juguete para unirse a la nivelación. Nos llenó de esperanza.",
    },
    {
      title: "Siembra y Reforestación Verde",
      date: "Día 15 de la Expedición",
      description:
        "Plantación exitosa de 80 plantones nativos (queñuales y molles), implementando un sistema de riego vecinal autogestionado.",
      participants: ["Sayri Ccama", "Andrés Vega", "Lucía Herrera"],
      lessonsLearned:
        "El suelo andino requiere abono profundo de compost local para soportar las raíces jóvenes.",
      emotionalNote:
        "Ver el primer queñual firme en la tierra limpia fue un momento de silencio y conexión profunda.",
    },
    {
      title: "Inauguración y Registro de Huella",
      date: "Día 20 de la Expedición",
      description:
        "Se culminó el mural cívico que narra la expedición. Se firmó el pacto de cuidado y mantenimiento entre la comunidad y las brigadas.",
      participants: ["Sayri Ccama", "Lucía Herrera", "Joaquín Ríos", "Camila Díaz"],
      lessonsLearned:
        "El impacto no termina al plantar; la gobernanza barrial es la clave para la longevidad del proyecto.",
      emotionalNote:
        "Los vecinos lloraron al ver el mural. Sintieron que por fin el barrio cobraba vida.",
    },
  ];
  ...
  const testimonials = [
    {
      quote:
        "Sentí que por primera vez mi barrio nos escuchaba. KUSQA nos unió para demostrar que la juventud puede sanar la tierra.",
      author: "Sayri Ccama",
      role: "Guardián de Cusco",
    },
    {
      quote:
        "No fue una tarde de voluntariado tradicional. Fue una convivencia que dejó una huella física y social en nuestra comunidad.",
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6 overflow-y-auto">
          ...
          >
            ...
            {/* Atmosphere Banner */}
            <div
              className={`h-56 sm:h-72 ${theme.gradient} relative text-white p-6 sm:p-10 flex flex-col justify-end overflow-hidden`}
            >
              ...
              <div className="relative z-10 space-y-2">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-stone-200/90 font-medium">
                  <span className="flex items-center gap-1">
                    <MapPin className="h-3.5 w-3.5" /> {mission.district}
                  </span>
                  <span>•</span>
                  <span className="flex items-center gap-1">
                ...
                <section className="space-y-3">
                  ...
                  <div className="p-5 rounded-2xl bg-amber-500/5 dark:bg-amber-500/10 border border-amber-500/15 text-xs sm:text-sm leading-relaxed text-muted-foreground">
                    <strong className="text-foreground">
                      Impacto duradero en {mission.district}:
                    </strong>{" "}
                    Las áreas verdes recuperadas han disminuido el desecho de basuras en un 60%,
                    logrando que la plaza sea un punto de encuentro seguro para la niñez local. La
                    comunidad conformó un comité de riego permanente de 6 familias voluntarias.
                  </div>
                </section>
                    ...
                    {timelineMilestones.map((node, idx) => {
                      ...
                      return (
                        <div key={idx} className="relative">
                          {/* Timeline Dot */}
                          <div
                            className={`absolute -left-[30px] top-1.5 h-3.5 w-3.5 rounded-full border-2 border-card ${theme.gradient} ring-4 ring-stone-100 dark:ring-stone-900`}
                          />
                          ...
                          <div className="bg-secondary/35 border border-border/20 rounded-2xl p-4 transition-all">
                            ...
                            >
                              <div>
                                <span className="text-[9px] uppercase font-black text-accent">
                                  {node.date}
                                </span>
                              ...
                              {isExpanded && (
                                ...
                                >
                                  <div>
                                    ...
                                    <div className="flex flex-wrap gap-1.5 mt-1">
                                      {node.participants.map((p) => (
                                        <span
                                          key={p}
                                          className="px-2 py-0.5 rounded-full bg-secondary text-foreground font-semibold"
                                        >
                                          {p}
                                        </span>
                                      ))}
                                  ...
                                  <div>
                                    ...
                                    <p className="mt-0.5 leading-relaxed italic">
                                      "{node.lessonsLearned}"
                                    </p>
                                  </div>
                                  <div className="p-2 bg-amber-500/5 dark:bg-amber-500/10 border border-amber-500/10 rounded-lg">
                                    <span className="font-black text-accent uppercase tracking-wider text-[8px]">
                                      Nota Emocional:
                                    </span>
                                    <p className="mt-0.5 leading-relaxed text-foreground/90 font-medium">
                                      "{node.emotionalNote}"
                                    </p>
                                  </div>
                                </motion.div>
                ...
                <section className="space-y-4">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    {testimonials.map((t, idx) => (
                      <div
                        key={idx}
                        className="p-4 rounded-2xl bg-card border border-border/80 text-xs italic leading-relaxed text-muted-foreground space-y-2"
                      >
                        <p>"{t.quote}"</p>
              ...
              <div className="space-y-6">
                ...
                <div className="p-5 rounded-3xl bg-card border border-border/80 space-y-3">
                  ...
                  <p className="text-[10px] text-muted-foreground leading-normal">
                    Evidencias visuales de la siembra cívica. Listo para integración futura con
                    Supabase Storage.
                  </p>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    {[
                      ...
                      <div
                        key={idx}
                        className="h-20 bg-secondary/80 hover:bg-secondary border border-border/20 rounded-xl flex flex-col items-center justify-center text-center p-1.5 transition-colors group cursor-not-allowed"
                      >
                        <span className="text-xl group-hover:scale-110 transition-transform duration-200 select-none">
                          {img.em}
                        </span>
                        <span className="text-[7px] font-black uppercase text-stone-400 mt-1 truncate max-w-full leading-none">
                          {img.l}
                        </span>
                      </div>
                    ))}
