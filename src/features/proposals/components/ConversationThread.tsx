...
import { DB_DEFAULTS, type ProposalComment } from "@/services/proposalContract";
...
export function ConversationThread({ proposalId }: ConversationThreadProps) {
  const userId = useCurrentUserId();
  const { data, isLoading, isError } = useProposalComments(proposalId, { page: 0, pageSize: 20 });
    ...
    <section className="space-y-4" aria-label="Conversación cívica">
      ...
      {!isLoading && !isError && topLevel.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-6">
          Aún no hay comentarios. Sé la primera en sumarte al diálogo.
        </p>
      )}
...
function CommentItem({ comment, currentUserId, onReply }: CommentItemProps) {
  const { mutate: edit, isPending: isEditing } = useEditComment();
  const { mutate: remove, isPending: isRemoving } = useDeleteComment();
